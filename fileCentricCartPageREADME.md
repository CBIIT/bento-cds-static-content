...
### The GC File Manifest
Within the General Commons (GC), users can easily generate a file manifest using the "Explore" dashboard. Data can be filtered by various attributes using a robust menu of facet filters. Participants, samples, and files matching selected filter criteria are displayed in a table and represented by color-coded widgets. After completing the desired level of filtering, users can add files associated with participants and samples to their "Selected Files" cart page. Clicking the "Download File Manifest" button generates a file manifest compatible with any CRDC Cloud Resource. The GC file manifest is timestamped at the time of download and includes essential metadata such as *drs_uri, file_name, Participant ID, Md5sum, Study Name, Accession, Sample ID, Study Access, File Type, Sex, Race, Primary Diagnosis, Sample Tumor Status, Sample Type, Organ or Tissue, Study Data Type, Library Strategy, Image Modality, Experimental Strategy, Library Layouts, License, File Size (in bytes), and User Comments.*

### Importing the GC File Manifest to the Cancer Genomics Cloud (CGC)
After downloading the file manifest from GC, users need not download any GC files locally. Instead, the file manifest provides instructions for Cloud Resources to access these files directly from cloud storage on demand.
<!-- PAGE BREAK --> 
**To export the GC file manifest to the Cancer Genomics Cloud (CGC), follow these steps:**
...
* From the “Import files from” dropdown menu, select “General Commons (GC)”.

* Upload the local copy of the GC file manifest generated from GC by dragging the file to the drop zone or clicking on the “Browse files” button .
...
This button facilitates the simple and immediate transfer of a file manifest generated in the GC to the Cancer Genomics Cloud (CGC). With just a single click, files are ready for analysis within the CGC. With a cohort of files of interest already derived via the GC Explore Dashboard and added to the “Selected Files” Cart page, clicking the “Export to Cancer Genomics Cloud” button will trigger the transfer of all necessary file metadata. Once authenticated with the CGC, users can digitally access the files from GC cloud storage within a CGC project.
<!-- PAGE BREAK --> 
